...
* AVR IoT board (doesn't matter whether it's a WG or AWS, we're not using outside forces)
    ...
    * PA3 aka. pin 3 in Arduino's world, is addressing the LED bar
...
* Some power solution:
    * The AVR-IoT can be supplied via the USB port or through the LiPo battery connector. 
    * The LED stripe can be powered through the 5V-pin of the mikroBus (see that the bridge next to it on the AVR IoT PCB is closed). That pin can provide only so much power though, you may want to consider a separate supply line (common GND though).
...
### arduino_secrets.h
Configure the wifi name and password here, and provide the coordinates for the MQTT broker. 

The IP is comma-separated, not by dots. The topic adresses, on my broker, look like `#define TOPIC_IMPORT "HA/cce7aa05f0f8/iotHANSensorPowerPlus/stat_t";`. 
